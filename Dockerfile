...
RUN \
  ...
  if [ -z ${LAZYLIBRARIAN_COMMIT+x} ]; then \
    LAZYLIBRARIAN_COMMIT=$(curl -sX GET "https://gitlab.com/api/v4/projects/44641666/repository/commits/master" \
  ...
  curl -o \
    ...
    "https://gitlab.com/gaizaharduz/LazyLibrarian/-/archive/{$LAZYLIBRARIAN_COMMIT}/LazyLibrarian-{$LAZYLIBRARIAN_COMMIT}.tar.gz" && \
  tar xf \
